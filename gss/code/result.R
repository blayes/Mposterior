...
for (dd in 1:10) {
  ...
  for (cc in 1:9) {
    res10 <- readRDS(paste0("/nfsscratch/Users/ssrivastva/gss/result/mpost/sub10/marg_cat_", cc, "_cv_", dd, ".rds"))
    mpost10 <- res10$res
    ...
    res20 <- readRDS(paste0("/nfsscratch/Users/ssrivastva/gss/result/mpost/sub20/marg_cat_", cc, "_cv_", dd, ".rds"))
    mpost20 <- res20$res
...
for (dd in 1:10) {
  ...
  for (cc in 1:36) {
    res10 <- readRDS(paste0("/nfsscratch/Users/ssrivastva/gss/result/mpost/sub10/joint_cat_", cc, "_cv_", dd, ".rds"))
    mpost10 <- res10$res
    ...
    res20 <- readRDS(paste0("/nfsscratch/Users/ssrivastva/gss/result/mpost/sub20/joint_cat_", cc, "_cv_", dd, ".rds"))
    mpost20 <- res20$res
...
for (ii in 1:10) {
  tbl <- unlist(read.table(paste0("/nfsscratch/Users/ssrivastva/gss/result/full/time_", ii, ".csv")))
  jtime[ii , 1] <- tbl
  t10 <- rep(0, 10)
  for (ss in 1:10) {
    t10[ss] <- unlist(read.table(paste0("/nfsscratch/Users/ssrivastva/gss/data/sub10/", ii, "/time_", ss, ".csv")))
  }
  jtime[ii , 2] <- mean(t10) + jtime[ii, 2]
  t20 <- rep(0, 20)
  for (ss in 1:20) {
    t20[ss] <- unlist(read.table(paste0("/nfsscratch/Users/ssrivastva/gss/data/sub20/", ii, "/time_", ss, ".csv")))
  }
...
for (ss in 1:10) {
  ...
  for (ii in 1:8) {
    for (jj in (ii + 1):9) {
      ...
      mp2 <- matrix(0.0, 2, 4)
      for (comp in 1:4) {
        op[, comp] <- quantile(jointOpost[[ss]][[cnt]][ , comp], probs = c(0.025 / 4, 1 - 0.025 / 4))
        mp1[, comp] <- quantile(jointSpost10[[ss]][[cnt]][ , comp], probs = c(0.025 / 4, 1 - 0.025 / 4))
        mp2[, comp] <- quantile(jointSpost20[[ss]][[cnt]][ , comp], probs = c(0.025 / 4, 1 - 0.025 / 4))
      }
